...
Changes on the development branch after `v0.2.1`.
...
- Added branch-specific documentation for ARMselection and updated the BGO,
  GeD, DAG, pipeline, installation, and repository guides to describe the
  current task graphs, runtimes, state files, products, and GCN behavior.
...
- Refactored the all-in-one installer for the current COSIflow Compose layout:
  ...
  - moved passwords and client credentials from tracked Compose YAML into the
    ignored `cosiflow/env/.env` file, with hidden prompts and `0600`
    permissions;
...
- Split the former mixed GeD workflow into a binned `cosidag_GeD` branch using
  `pipeline_config.yaml` and an unbinned `cosidag_ARMselection` branch using
  `armsel_pipeline_config.yaml`, preventing concurrent state-file writes.
- Refactored the initialization DAG and expanded inline documentation for the
  BGO, GeD, and ARMselection configuration, task wiring, monitoring rules, and
  concurrency settings.
- Updated plot identity metadata and documented the filenames currently
  generated by all three scientific branches.
- Corrected the COSIpy requirements header to match its configured Python 3.12
  runtime and clarified the pinned official `cosipy.nonimaging` dependency.
